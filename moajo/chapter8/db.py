...
def calc_weights() -> LogisticRegression:
    """
    既存のやつでロジスティック回帰
    :return:
    """
    ...
    for data in c.find():
        label = data["label"]
        features = data["feats"]
        sentiments.append({"features": si.sentence_to_vec(features), "label": label})

    lr.calc_weights(sentiments=sentiments)
    return lr


def calc_weights2() -> (Skl, list):
    """
    sk-learnでロジスティック回帰
    :return:
    """
    ...
    for data in c.find():  # 抽出された特徴量から全データを学習
        ...
        for t in sparse:
            a[t] = 1
        # sentiments.append({"features": si.sentence_to_vec(features), "label": label})
        sentiments.append([a, 1 if label == "+1" else 0])
        X.append(a)
